__author__ = 'Tom'
# Notes
# MS[7].midlink[0, 1] should be true
# MS[0].midlink[0, 1:3] should be true
# Need to make sure columns contain only one True
...
class clos_network:
    def __init__(self, n1, n2, r1, r2, x):
    ...
    def multicast_generator(self, input_switch_num, input_node, output_switch_nums=(list, tuple)):
        outs_set = set(output_switch_nums)
        ...
        for g in range(1, self.mid_switch_max):
            combine_iter = itertools.combinations([i for i in range(self.num_mid_switches)], g)
            while True:
                try:
                    indeces = combine_iter.next()
                ...
                for i in indeces:
                    # print "index = ", i
                    if len(outs_int & self.middle_switches[i].free_outs(set)) is not 0:
                        mid_pairs.update({i: list(outs_int & self.middle_switches[i].free_outs(set))})
                        # print colored("mid_pairs = ", "green"), mid_pairs
                    union |= self.middle_switches[i].free_outs(set)
                    outs_int -= self.middle_switches[i].free_outs(set)
                ...
                if outs_set.issubset(union):
                    # print "bleh"
                    # print mid_pairs
                    cxn_color = self.input_switches[input_switch_num].colors[input_node]
                    # print cxn_color
                    # print "mid_pairs.keys() = ", mid_pairs.keys()
                    self.input_switches[input_switch_num].connect(input_node, mid_pairs.keys())
                    # print "bauble"
                    for i in mid_pairs.keys():
                        ...
                        for j in mid_pairs[i]:
                            ...
                            self.output_switches[j].inherit_color(j, cxn_color)
                            self.output_switches[j].connect(j, self.output_switches[j].free_outs()[0])
                ...
                if index_counter & g == 0:
                    union = set()
                    outs_int = set(output_switch_nums)
                    mid_pairs = dict()
            if mcast_est:
                # Mutlicast established, do not increment combination length
                break
        return mcast_est
...
test_network = clos_network(n1, n2, r1, r2, x)
