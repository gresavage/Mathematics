...
def utrue(x, y, t):
    return (x**2.0-2.0*pi*x)*(y**2.0-2.0*pi*y)*t

def forcing(x, y, t, c):
    return (x**2.0-2.0*pi*x)*(y**2.0-2.0*pi*y)-c*(2.0*t*x*(x-2.0*pi)+2.0*t*y*(y-2.0*pi))
...
PlotEvery = TimeSteps  # Decide when to plot
...
# print "x[0]: ", x[0]
# print "y[grdcellsy-1]: ", y[grdcellsy-1]
Y, X = meshgrid(x, y)
...
fnpo = zeros((unknowns, 1), float)         # Forcing function f_n+1/2: Used instead of f_n, and f_n+1 to save on memory
FNPO = zeros((grdcellsx, grdcellsy), float)# Forcing function f_n+1/2 matrix
...
# jL = 0
# iL = 0
for j in xrange(grdcellsy):
    for i in xrange(grdcellsx):
        # set entry posiion
        ipo = (i+1) % grdcellsx
        jpo = (j+1) % grdcellsy
        imo = (i-1) % grdcellsx
